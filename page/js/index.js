  ...
  methods: {
    refresh: function () {
      var totalPage = Math.floor(
        (pageTools.total + pageTools.limit - 1) / pageTools.limit
      );
      pageTools.pageList = [];
      pageTools.pageList.push({ text: "首页", pageNum: 1 });
      if (pageTools.nowPage - 2 > 0) {
        pageTools.pageList.push({
          text: pageTools.nowPage - 2,
          pageNum: pageTools.nowPage - 2,
        });
      }
      if (pageTools.nowPage - 1 > 0) {
        pageTools.pageList.push({
          text: pageTools.nowPage - 1,
          pageNum: pageTools.nowPage - 1,
        });
      }
      pageTools.pageList.push({
        text: pageTools.nowPage,
        pageNum: pageTools.nowPage,
      });
  ...
  computed: {
    changePage: function () {
      return function (pageNum) {
        ...
        axios({
          url:
            "/blog/getBlogByPage?offset=" +
            (pageNum - 1) * pageTools.limit +
            "&limit=" +
            pageTools.limit,
          method: "get",
        }).then((resp) => {
